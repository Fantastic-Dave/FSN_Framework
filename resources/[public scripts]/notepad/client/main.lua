...
RegisterNUICallback('escape', function(data, cb)
    ...
    TriggerEvent("notepad:CloseNotepad")
end)
...
RegisterNUICallback('droppingEmpty', function(data, cb)
    print('You cant drop an empty notepad')
end)
...
RegisterNetEvent("notepad:OpenNotepadGui")
AddEventHandler("notepad:OpenNotepadGui", function()
    if not isUiOpen then
        openGui()
    end
end)

RegisterNetEvent("notepad:CloseNotepad")
AddEventHandler("notepad:CloseNotepad", function()
    SendNUIMessage({
        action = 'closeNotepad'
    })
...
RegisterNetEvent('notepad:note')
AddEventHandler('notepad:note', function()
    ...
    if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
        ...
        if ( IsEntityPlayingAnim( player, ad, "base", 3 ) ) then 
            ...
            AttachEntityToEntity(prop, player, GetPedBoneIndex(player, 18905), 0.1, 0.02, 0.05, 10.0, 0.0, 0.0, true, true, false, true, 1, true) -- notepadpad
...
RegisterNetEvent('notepad:updateNotes')
AddEventHandler('notepad:updateNotes', function(serverNotesPassed)
    ...
    while true do
        ...
        if #TestLocalTable == 0 then
            ...
            if distance < 2.0 then
                ...
                Util.DrawText3D(TestLocalTable[closestNoteId]["x"],TestLocalTable[closestNoteId]["y"],TestLocalTable[closestNoteId]["z"]-0.2, "~g~E~s~ to read, ~r~G~s~ to destroy", {255, 255, 255, 255}, 0.25)
