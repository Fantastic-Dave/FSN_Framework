...
RegisterServerEvent("fsn_bennys:buttonSelected")
AddEventHandler("fsn_bennys:buttonSelected", function(name, button)
	...
	if button.price then -- check if button have price
		if button.price <= mymoney then
			TriggerClientEvent("fsn_bennys:buttonSelected", source,name, button, true)
			mymoney  = mymoney - button.price
		else
			TriggerClientEvent("fsn_bennys:buttonSelected", source,name, button, false)
		end
	end
end)

RegisterServerEvent("fsn_bennys:finished")
AddEventHandler("fsn_bennys:finished", function(veh)
